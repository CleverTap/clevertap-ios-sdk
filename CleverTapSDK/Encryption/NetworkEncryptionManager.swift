...
@objc public class NetworkEncryptionManager: NSObject {
    ...
    func getOrGenerateSessionKey() -> SymmetricKey {
        return keyQueue.sync {
            ...
            CTLogger.logWithLevel(CTLogger.getDebugLevel(), type: CTLogType.debug.rawValue, message: "Generated new session key.")
            sessionKey = newKey
            return newKey
        }
    }
    ...
    @objc public func getSessionKeyBase64() -> String? {
        if #available(iOS 13.0, tvOS 13.0, *) {
            ...
            CTLogger.logWithLevel(CTLogger.getDebugLevel(), type: CTLogType.debug.rawValue, message: "Encryption in transit is only available from iOS 13 and later.")
            return nil
        }
    }
    ...
    @objc public func encrypt(object: Any) -> [String: Any] {
        if #available(iOS 13.0, tvOS 13.0, *) {
            do {
                ...
                CTLogger.logWithLevel(CTLogger.getDebugLevel(), type: CTLogType.debug.rawValue, message: "Encryption in transit error: \( error.localizedDescription)")
                return [:]
            }
        } else {
            CTLogger.logWithLevel(CTLogger.getDebugLevel(), type: CTLogType.debug.rawValue, message: "Encryption in transit is only available from iOS 13 and later.")
    ...
    @objc public func decrypt(responseData: Data) -> Data {
        if #available(iOS 13.0, tvOS 13.0, *) {
            ...
            do {
                CTLogger.logWithLevel(CTLogger.getDebugLevel(), type: CTLogType.debug.rawValue, message: "Encrypted Response: \(response)")
                ...
                CTLogger.logWithLevel(CTLogger.getDebugLevel(), type: CTLogType.debug.rawValue, message: "Decryption in transit error: \(error.localizedDescription)")
                return Data()
            }
        } else {
            CTLogger.logWithLevel(CTLogger.getDebugLevel(), type: CTLogType.debug.rawValue, message: "Encryption in transit is only available from iOS 13 and later.")
