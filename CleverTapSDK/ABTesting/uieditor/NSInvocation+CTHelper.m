...
static void CTFree(void *p){
    if (p) {
        free(p);
    }
}

static void *CTAllocBufferForObjCType(const char *objCType) {
    void *buffer = NULL;
    ...
    if (result != 0) {
        CleverTapLogStaticDebug(@"Error allocating aligned memory: %s", strerror(result));
    }
...
- (void)ct_setArgument:(id)argumentValue atIndex:(NSUInteger)index {
    ...
    else if ([argumentValue isKindOfClass:[NSValue class]])
    {
        ...
        void *buffer = CTAllocBufferForObjCType(valueArgument.objCType);
...
- (void)ct_setArgumentsFromArray:(NSArray *)argumentArray {
    ...
    for (id argument in argumentArray) {
        ...
        [self ct_setArgument:argument atIndex:argumentIndex];
        ++i;
    }
}
...
- (id)ct_returnValue {
    ...
    void *buffer = CTAllocBufferForObjCType(objCType);
