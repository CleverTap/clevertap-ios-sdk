...
static NSDictionary *CTCATransform3DCreateDictionaryRepresentation(CATransform3D transform)
{
    return @{
             @"m11": @(transform.m11),
             @"m12": @(transform.m12),
...
static BOOL CTCATransform3DMakeWithDictionaryRepresentation(NSDictionary *dictionary, CATransform3D *transform)
{
    if (transform) {
...
@implementation CTNSDictionaryFromCATransform3DValueTransformer
...
- (id)transformedValue:(id)value {
    if ([value respondsToSelector:@selector(CATransform3DValue)]) {
        return CTCATransform3DCreateDictionaryRepresentation([value CATransform3DValue]);
    }
    return @{};
}
...
- (id)reverseTransformedValue:(id)value {
    ...
    if ([value isKindOfClass:[NSDictionary class]] && CTCATransform3DMakeWithDictionaryRepresentation(value, &transform)) {
