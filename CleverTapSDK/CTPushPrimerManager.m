...
- (instancetype)initWithConfig:(CleverTapInstanceConfig *)config inappsController:(CTInappsController*)inappsControllerObj dispatchQueueManager:(CTDispatchQueueManager*)dispatchQueueManager {
    
    if ((self = [super init])) {
        self.config = config;
        inappsController = inappsControllerObj;
    ...
    if (@available(iOS 10.0, *)) {
        [self getNotificationPermissionStatusWithCompletionHandler:^(UNAuthorizationStatus status) {
            if (status == UNAuthorizationStatusNotDetermined || status == UNAuthorizationStatusDenied) {
                [self->inappsController prepareNotificationForDisplay:json];
            } else {
...
- (void)showFallbackToSettingsAlertDialog {
    ...
    [inappsController prepareNotificationForDisplay:alertSettings];
}
...
- (void)openAppSettingsForPushNotification {
    ...
    [CTUtils runSyncMainQueue:^{
        [CTUtils openURL:url forModule:@"PushPermission"];
    }];
}
#endif
