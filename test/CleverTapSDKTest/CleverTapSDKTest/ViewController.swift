...
class ViewController: UIViewController, CleverTapInboxViewControllerDelegate, WKNavigationDelegate, WKScriptMessageHandler, UIScrollViewDelegate, CleverTapInAppNotificationDelegate {
...
//        ctConfig.disableAppLaunchedEvent = true
...
//        NSLog("additional CleverTap instance created for accountID: %@", cleverTapAdditionalInstance.config.accountId)
        ...
        CleverTap.sharedInstance()?.recordEvent("Test iOS - Simple event3")
        
//                CleverTap.sharedInstance()?.recordEvent("Cover Image ios")
//                CleverTap.sharedInstance()?.recordEvent("Cover Portrait")
